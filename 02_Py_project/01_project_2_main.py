...
while True:
  ...
  user_number = (
      input("Guess the number from range 1 to 100\n\n-------> ")).strip()

  # making sure that user input must be an int
  if user_number == '' or user_number.isspace() or user_number.isalpha(
  ) or ' ' in user_number:
    print("\nALERT!!! Please input an int number")
    continue

  elif user_number.isalnum():
    user_number = int(
        user_number)  # converting str version of user_numbe into int version
